...
CLISetup::registerSetup("sql", new class extends SetupScript
{
    ...
    public function generate(array $ids = []) : bool
    {
        ...
        */
        ...
        CLI::write('[sound] - sounds main data');
        ...
        while ($sounds = DB::Aowow()->select($query, $j * CLISetup::$SQL_BATCH, CLISetup::$SQL_BATCH))
        {
            ...
            foreach ($sounds as $s)
            {
                ...
                */
                ...
                {
                    CLI::write('[sound] Group '.str_pad('['.$s['id'].']', 7).' '.CLI::bold($s['name']).' contains no sound files! Skipping...', CLI::LOG_WARN);
                    continue;
                }
                else if ($fileSets)
        ...
        CLI::write('[sound] - linking to creatures');
        ...
        DB::Aowow()->query(
           'INSERT INTO ?_creature_sounds
            SELECT      cdi.`id`,
                        ...
                        0,                                  -- transform
                        0                                   -- transformanimated
            FROM        dbc_creaturedisplayinfo cdi
            LEFT JOIN   dbc_creaturemodeldata cmd  ON  cmd.`id` = cdi.`modelId`
            LEFT JOIN   dbc_creaturesounddata csdA ON csdA.`id` = cdi.`creatureSoundId`
            LEFT JOIN   dbc_creaturesounddata csdB ON csdB.`id` = cmd.`creatureSoundId`
            LEFT JOIN   dbc_npcsounds ns           ON   ns.`id` = cdi.`npcSoundId`'
        );
        ...
        CLI::write('[sound] - linking to spells');
